<template>
  <div class="proposal-wrapper position-relative" style="font-family: 'Noto Sans', sans-serif">
    <!-- 背景圖 -->
    <img
      src="/proposal-bg2.png"
      class="position-absolute top-0 start-0"
      style="z-index: -1; width: 100%; height: 80vh; object-fit: cover"
    />

    <!-- 標題區 -->
    <div class="proposal-header text-white">
      <img
        src="/proposal-deco.png"
        class="position-absolute deco-icon"
        style="top: 60px; left: 120px; width: 280px"
      />
      <div
        class="container py-2 d-flex flex-column align-items-start text-start"
        style="padding-top: 6rem !important; max-width: 900px; margin: 0 auto"
      >
    ...
    <!-- 表單內容 -->
    <div
      class="proposal-body py-5"
      style="
        background-image: url('/proposal-bg.png');
      ...
      <div class="container" style="padding-top: 10rem; max-width: 900px; margin: 0 auto">
        <form @submit.prevent="submitForm">
          ...
          <div class="form-section mb-5">
            ...
            <div class="mb-3">
              ...
              <div class="icon-overlay-wrapper">
                <span class="calendar-overlay" style="left: 10px; font-size: 0.9rem; color: #5f6368"
                  >NT$</span
                >
                <input
            ...
            <div class="faq-box faq-new shadow-sm">
              ...
              <div class="mb-3">
                <textarea
                  ...
                  style="font-weight: 300"
                  rows="3"
                  placeholder="請回答上方問題，提供具體資訊或執行方式說明。"
              ...
              <div class="text-end">
                <button
                  type="button"
                  class="addbtn btn-outline-dark rounded-pill px-2.5 py-1"
                  @click="addFaq"
                >
                  ＋ 新增
                </button>
          ...
          <div class="text-center mt-4">
            <button type="submit" class="btn btn-danger rounded-pill px-6 py-2">
              下一步：填寫回饋方案
            </button>
...
async function uploadFile(file) {
  try {
    ...
    const res = await axios.post(
      'https://lovia-backend-xl4e.onrender.com/api/v1/uploads/image',
      formData,
      {
        headers: {
...
const formFaqs = ref([
  ...
  },
  {
    ...
    answer:
      '目前本專案為整體性規劃，未提供指定對象功能。不過您將會收到我們在三所學校執行的完整成果紀錄。',
  },
  {
    question: '這個協會真的可信嗎',
    answer:
      '我們為正式立案之公益團體，協會資訊可於內政部網站查詢。我們也會在專案頁面展示過往成果與媒體報導。',
  },
])
...
async function loadProjectData(id) {
  try {
    ...
    // FAQ 預設處理
    formFaqs.value = Array.isArray(project.faq)
      ? project.faq.map((faq) => ({
